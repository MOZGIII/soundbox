                    ...
                    match stream_type {
                        StreamType::Input => {
                            ...
                            match stream_inner.sample_format {
                                ...
                                SampleFormat::F32 => {
                                    // The buffer to read into.
                                    let mut buffer: Vec<f32> = iter::repeat(mem::uninitialized())
                                        .take(available)
                                        .collect();
                                    let frames_or_err = alsa::snd_pcm_readi(
                                        stream_inner.channel,
                                        buffer.as_mut_ptr() as *mut _,
                                        available as _,
                                    );
                                    if frames_or_err < 0 {
                                        let _ = alsa::snd_pcm_recover(
                                            stream_inner.channel,
                                            frames_or_err as i32,
                                            0,
                                        );
                                    }
                                    check_errors(frames_or_err as _).expect("snd_pcm_readi error");
